...
#pragma warning(disable: 4100)		// Get rid of (erroneous) 'unreferenced formal parameter' warning
	...
	AE_NO_TSAN weak_atomic(nullptr_t) : value(nullptr) {  }
        ...
        AE_NO_TSAN weak_atomic(weak_atomic&& other) : value(__mov__(other.load())) {  }
#ifdef AE_VCPP
#pragma warning(pop)
#endif
	...
	AE_FORCEINLINE content_t load() const AE_NO_TSAN { return value; }

	AE_FORCEINLINE content_t fetch_add_acquire(content_t increment) AE_NO_TSAN
	{
#if defined(AE_ARCH_X64) || defined(AE_ARCH_X86)
		if (sizeof(content_t) == 4) return _InterlockedExchangeAdd((long volatile*)&value, (long)increment);
#if defined(_M_AMD64)
		else if (sizeof(content_t) == 8) return _InterlockedExchangeAdd64((long long volatile*)&value, (long long)increment);
#endif
#else
#error Unsupported platform
#endif
		assert(false && "content_t must be either a 32 or 64 bit type");
		return value;
	}

	AE_FORCEINLINE content_t fetch_add_release(content_t increment) AE_NO_TSAN
	{
#if defined(AE_ARCH_X64) || defined(AE_ARCH_X86)
		if (sizeof(content_t) == 4) return _InterlockedExchangeAdd((long volatile*)&value, (long)increment);
#if defined(_M_AMD64)
...
#error Unsupported platform! (No semaphore wrapper available)
        ...
        private:
            ...
            bool waitWithPartialSpinning(std::int64_t timeout_usecs = -1) AE_NO_TSAN
            {
                ...
                // wasn't signaled enough times for us too since transform. If it was, we
